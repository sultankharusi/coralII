    ...
    for center in plate_centers.keys():
        for i in keys_:
            if not vehicle_box[i]['plate']:
                vehicle_box[i]['plate_hits']+=1
                box = vehicle_box[i]['box']
                #print(box)
                if is_inside(center, box) and vehicle_box[i]['plate_hits'] > 5:
...
def get_scaled_box(box, yscale=0.7, xscale=0.27, box_only = False):
...
def plate_inference(plate,V_box,yscale=0.7,xscale=0.27): # OmanOil yscale 1.22, xscale 0.36 # Video is yscale=0.96,xscale=0.256
...
def main():
    ...
    while True:
        ...
        if not ret:
            break
        cv2_im_cropped = frame[350:896,250:1650] #170:504 (1.15), 160:1320 (0.33) *Orig* 190:504,235:1300 This is a global variable and shall never be altered, so cv2_im_cropped is never augmented after this point
                                                #546(0.7) * 1400 (0.27)   Pump3
        ...
        if objs:
            ...
            plate_list = {}
            for i in objs:
                if i[0] in filtered and i[2][3] >= 180 and i[2][3] <=360:    
